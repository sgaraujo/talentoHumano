...
export function CompanySettingsPage() {
  ...
  const remove = async (company: Company) => {
    if (role !== 'admin' || !window.confirm(`¿Eliminar definitivamente ${company.name}? Solo hazlo si no tiene relaciones ni proyectos.`)) return;
  ...
  return <div className="p-4 sm:p-6 bg-gray-50 min-h-screen space-y-5">
    ...
    <div className="rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-800"><b>Esta pantalla configura el catálogo.</b> La dotación, nómina, proyectos e indicadores se consultan desde el módulo Empresas y no se guardan aquí.</div>
    ...
    <Dialog open={open} onOpenChange={setOpen}><DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto"><DialogHeader><DialogTitle>{selected ? 'Configurar empresa' : 'Nueva empresa'}</DialogTitle></DialogHeader><div className="grid sm:grid-cols-2 gap-4 py-2">
      ...
      <div className="sm:col-span-2"><Label>Módulos habilitados</Label><div className="grid sm:grid-cols-3 gap-2 mt-2">{([['humanResources','Talento Humano'],['accounting','Contabilidad'],['communications','Comunicaciones']] as const).map(([field,label]) => <button type="button" key={field} onClick={() => toggleModule(field)} className={`rounded-lg border px-3 py-2 text-sm font-medium ${form.modules?.[field] ? 'bg-green-50 border-green-300 text-green-700' : 'bg-gray-50 text-gray-400'}`}>{form.modules?.[field] ? '✓ ' : ''}{label}</button>)}</div></div>
