    ...
    <div className="p-4 sm:p-6 bg-gray-50 min-h-screen space-y-5">
      ...
      <div className="bg-white border rounded-xl overflow-hidden">
        <div className="p-4 border-b space-y-3">
          <div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" /><Input className="pl-9" placeholder="Buscar nombre, cédula, empresa o proyecto…" value={search} onChange={event => setSearch(event.target.value)} /></div>
        ...
        {loading ? <div className="py-16 text-center text-gray-400"><Loader2 className="w-7 h-7 animate-spin mx-auto mb-2" />Cargando expedientes…</div> : (
          <div className="overflow-x-auto max-h-[55vh] overflow-y-auto">
            <table className="w-full text-sm"><thead className="sticky top-0 bg-gray-50 text-left text-xs text-gray-500"><tr><th className="px-4 py-3">Persona</th><th className="px-4 py-3">Cédula</th><th className="px-4 py-3">Empresa / proyecto</th><th className="px-4 py-3">Relaciones</th><th className="px-4 py-3">Acceso</th></tr></thead>
              <tbody className="divide-y">{filtered.slice(0, 1000).map(item => <tr key={item.id} onClick={() => setSelectedEmployeeId(item.id)} className="hover:bg-green-50/60 cursor-pointer transition-colors" title="Abrir expediente"><td className="px-4 py-3 font-medium text-gray-700">{item.fullName}</td><td className="px-4 py-3 font-mono text-xs text-gray-500">{item.documentNumber}</td><td className="px-4 py-3 text-gray-500"><p>{item.companies.join(', ') || '—'}</p><p className="text-xs text-gray-400">{item.projects.join(', ') || 'Sin proyecto'}</p></td><td className="px-4 py-3"><span className="text-green-700">{item.activeRelations} activas</span><span className="text-gray-400"> · {item.retiredRelations} históricas</span></td><td className="px-4 py-3">{item.identityUserId ? <span className="text-green-700 bg-green-50 px-2 py-1 rounded-full text-xs">Vinculado</span> : <span className="text-purple-700 bg-purple-50 px-2 py-1 rounded-full text-xs">Sin acceso</span>}</td></tr>)}</tbody>
            </table>
